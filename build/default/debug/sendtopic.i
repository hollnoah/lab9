...
;BANK 3
    BSF STATUS,5 ;((ADCON0) and 07Fh), 1 TO BANK 3
    BSF STATUS,6 ;((ADCON0) and 07Fh), 1 TO BANK 3
...
;BANK 2
    BSF STATUS,6 ;((ADCON0) and 07Fh), 1 TO BANK 2
    BCF STATUS,5 ;((ADCON0) and 07Fh), 1 TO BANK 2
    CLRF CM2CON1 ;SET ALL BITS TO 0

;BANK 1
    BSF STATUS,5 ;((ADCON0) and 07Fh), 1 TO BANK 1
    BCF STATUS,6 ;((ADCON0) and 07Fh), 1 TO BANK 1
    ...
    MOVLW 0xA6 ;TRANSMIT ENABLE, ASYNCH, ((TXSTA) and 07Fh), 4 BREAK COMPLETE, HIGH SPEED, TSR EMPTY
    MOVWF TXSTA ;TRANSMIT STATUS & CONTROL REGISTSER

;BANK 0
    BCF STATUS,5 ;((ADCON0) and 07Fh), 1 TO BANK 0
    BCF STATUS,6 ;((ADCON0) and 07Fh), 1 TO BANK 0
...
CHECKDOLLAR:
   ...
   BTFSS STATUS,2 ;CHECKS IF ((STATUS) and 07Fh), 2 FLAG IS SET (IF THEY MATCH)
...
WAIT_TXIF_HI:
    BANKSEL TXSTA
    BTFSS TXSTA,1 ; ((PIR1) and 07Fh), 4
    GOTO WAIT_TXIF_HI
    BANKSEL TXREG
    MOVWF TXREG

    ; Wait again until ((PIR1) and 07Fh), 4 is set (byte has moved to TSR)
WAIT_TXIF_DONE:
    BANKSEL TXSTA
    BTFSS TXSTA,1
    GOTO WAIT_TXIF_DONE
